...
void Application::Init(String a_sApplicationName, uint a_uWidth, uint a_uHeight, bool a_bFullscreen, bool a_bBorderless)
{
	...
	m_pCameraMngr->SetPositionTargetAndView(
		vector3(0.0f, 0.0f, 8.0f),//Camera position
...
void Application::ReadConfig(void)
{
	...
	while (reader.ReadNextLine() == OUT_RUNNING)
	{
		...
		// Reads lines from the .ini with the 'Stop' keyword, adding the data to a vector
		else if (sWord == "Stop:") {
			float fXPos;
			float fYPos;
			float fZPos;
			sscanf_s(reader.m_sLine.c_str(), "Stop: [%f,%f,%f]", &fXPos, &fYPos, &fZPos);
			vector3 fPos = vector3(fXPos, fYPos, fZPos);
			m_vStopList.push_back(fPos);
			m_uTotalStops++;
		}
...
void Application::WriteConfig(void)
{
	...
	// Create new stops based on the default values (same as the example practical)
	fprintf(pFile, "\n\nStop: [%.1f,%.1f,%.1f]", -3.0, -3.0, -3.0);
	fprintf(pFile, "\nStop: [%.1f,%.1f,%.1f]", -3.0, 3.0, 0.0);
	fprintf(pFile, "\nStop: [%.1f,%.1f,%.1f]", -2.0, -2.0, -3.0);
	fprintf(pFile, "\nStop: [%.1f,%.1f,%.1f]", -2.0, 2.0, 0.0);
	fprintf(pFile, "\nStop: [%.1f,%.1f,%.1f]", -1.0, -1.0, -3.0);
	fprintf(pFile, "\nStop: [%.1f,%.1f,%.1f]", -1.0, 1.0, 0.0);
	fprintf(pFile, "\nStop: [%.1f,%.1f,%.1f]", 0.0, 0.0, 0.0);
	fprintf(pFile, "\nStop: [%.1f,%.1f,%.1f]", 1.0, -1.0, 0.0);
	fprintf(pFile, "\nStop: [%.1f,%.1f,%.1f]", 1.0, 1.0, -3.0);
	fprintf(pFile, "\nStop: [%.1f,%.1f,%.1f]", 2.0, -2.0, 0.0);
	fprintf(pFile, "\nStop: [%.1f,%.1f,%.1f]", 2.0, 2.0, -3.0);
